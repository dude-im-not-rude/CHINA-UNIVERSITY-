...
async function upsert(universityId, universityName, record) {
  ...
  if (source[0]) await db`UPDATE sources SET last_checked_at=now(), verification_status='verified', notes='Program discovered on university official website.' WHERE id=${source[0].id}`;
  else await db`INSERT INTO sources (program_id, source_name, source_url, source_type, is_official, last_checked_at, verification_status, notes) VALUES (${id}, ${universityName}, ${record.url}, 'official', true, now(), 'verified', 'Program discovered on university official website.')`;
  return true;
}
...
console.log(`[OFFICIAL-ALL] ${universities.length} universities queued; no hard-coded university limit.`);
let total = 0;
for (const u of universities) {
  ...
  while (queue.length && pages < MAX_PAGES_PER_UNIVERSITY) {
    ...
    try {
      const p = await fetchText(url); pages++; const text = clean(p.html); const d = degree(text, p.url);
      if (d && likely(p.url) && /(program|programme|course|major|degree|bachelor|master|undergraduate|graduate)/i.test(`${p.url} ${text.slice(0, 5000)}`)) candidates.set(p.url, { html: p.html, text, degree: d });
      for (const [link, anchor] of links(p.html, p.url)) if (sameSite(link, u.official_website) && !seen.has(link) && likely(link, anchor)) queue.push(link);
      await sleep(100);
    } catch (e) { console.warn(`[OFFICIAL-ALL] ${u.name_english}: ${url} :: ${e.message}`); }
  }
  let imported = 0;
  for (const [url, c] of candidates) { const record = { name: title(c.html, url), degree: c.degree, language: language(c.text), duration: duration(c.text), tuition: tuition(c.text), year: year(c.text), deadline: deadline(c.text), url }; try { if (await upsert(u.id, u.name_english, record)) imported++; } catch (e) { console.warn(`[OFFICIAL-ALL] ${u.name_english}: import failed ${record.name} :: ${e.message}`); } }
  total += imported;
  console.log(`[OFFICIAL-ALL] ${u.name_english}: ${pages} pages, ${candidates.size} candidates, ${imported} imported/updated`);
}
