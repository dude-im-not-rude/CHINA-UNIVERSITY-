...
export async function getUniversities(filters = {}) {
  ...
  if (filters.q) add(`(u.name_english ILIKE '%' || $VALUE || '%' OR COALESCE(u.name_chinese,'') ILIKE '%' || $VALUE || '%' OR COALESCE(u.city,'') ILIKE '%' || $VALUE || '%' OR EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND (p.program_name ILIKE '%' || $VALUE || '%' OR COALESCE(p.field_of_study,'') ILIKE '%' || $VALUE || '%')))`, filters.q);
  if (filters.university) add(`u.name_english ILIKE '%' || $VALUE || '%'`, filters.university);
  if (filters.degree) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND lower(p.degree_level)=lower($VALUE))`, filters.degree);
  ...
  if (filters.major) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND (p.field_of_study ILIKE '%' || $VALUE || '%' OR p.program_name ILIKE '%' || $VALUE || '%'))`, filters.major);
  if (filters.program) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND p.program_name ILIKE '%' || $VALUE || '%')`, filters.program);
  if (filters.language) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND lower(COALESCE(p.language, CASE WHEN p.english_taught THEN 'English' ELSE '' END))=lower($VALUE))`, filters.language);
  if (filters.english === 'true') where.push(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND p.english_taught=true)`);
  ...
  if (filters.csca === 'true') where.push(`EXISTS (SELECT 1 FROM programs p JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND ar.csca_required=true)`);
  if (filters.csca === 'false') where.push(`EXISTS (SELECT 1 FROM programs p LEFT JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND COALESCE(ar.csca_required,false)=false)`);
  ...
  if (filters.intake) add(`EXISTS (SELECT 1 FROM programs p JOIN intakes i ON i.program_id=p.id WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND i.intake_name ILIKE '%' || $VALUE || '%')`, filters.intake);
  if (filters.open === 'true') where.push(`EXISTS (SELECT 1 FROM programs p JOIN intakes i ON i.program_id=p.id WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND (i.application_status ILIKE '%open%' OR (i.application_open_date IS NOT NULL AND CURRENT_DATE >= i.application_open_date AND (i.application_deadline IS NULL OR CURRENT_DATE <= i.application_deadline))))`);
  return db.query(`SELECT u.id, u.name_english, u.name_chinese, u.short_name, u.university_type, u.city, u.province, u.country, u.official_website, u.admissions_website, u.c9, u.project_985, u.project_211, u.double_first_class, u.university_description, u.logo_url, u.cover_image_url,
    EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND p.english_taught=true) AS has_english_program,
    ...
    EXISTS (SELECT 1 FROM programs p JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM} AND ar.csca_required=true) AS has_csca,
    (SELECT COUNT(*)::int FROM programs p WHERE p.university_id=u.id AND ${ACTIVE_PROGRAM}) AS program_count
...
export async function getUniversityById(id) {
  ...
  const programs = await db.query(`SELECT p.*, ar.csca_required, ar.csca_subjects, ar.english_requirement, ar.ielts_min, ar.toefl_min, ar.hsk_requirement, ar.minimum_percentage, ar.mathematics_required,
    ...
    FROM programs p LEFT JOIN admission_requirements ar ON ar.program_id=p.id LEFT JOIN intakes i ON i.program_id=p.id WHERE p.university_id=$1 AND ${ACTIVE_PROGRAM} GROUP BY p.id, ar.id ORDER BY p.degree_level, p.program_name`, [id]);
...
export async function getProgramById(id) {
  ...
  const rows = await db.query(`SELECT p.*, u.id AS university_id, u.name_english AS university_name, u.city, u.province, u.university_type, u.c9, u.project_985, u.project_211, u.double_first_class, u.official_website, u.admissions_website, u.university_description,
    ...
    FROM programs p JOIN universities u ON u.id=p.university_id LEFT JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.id=$1 AND ${ACTIVE_PROGRAM} LIMIT 1`, [id]);
  if (!rows[0]) return null;
