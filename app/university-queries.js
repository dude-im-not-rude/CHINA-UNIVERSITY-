...
export async function getUniversities(filters = {}) {
  ...
  if (filters.q) add(`(u.name_english ILIKE '%' || $VALUE || '%' OR COALESCE(u.name_chinese,'') ILIKE '%' || $VALUE || '%' OR COALESCE(u.city,'') ILIKE '%' || $VALUE || '%' OR EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND (p.program_name ILIKE '%' || $VALUE || '%' OR COALESCE(p.field_of_study,'') ILIKE '%' || $VALUE || '%')))`, filters.q);
  if (filters.university) add(`u.name_english ILIKE '%' || $VALUE || '%'`, filters.university);
  if (filters.degree) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND lower(p.degree_level)=lower($VALUE))`, filters.degree);
  ...
  if (filters.major) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND (p.field_of_study ILIKE '%' || $VALUE || '%' OR p.program_name ILIKE '%' || $VALUE || '%'))`, filters.major);
  if (filters.program) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND p.program_name ILIKE '%' || $VALUE || '%')`, filters.program);
  if (filters.language) add(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND lower(COALESCE(p.language, CASE WHEN p.english_taught THEN 'English' ELSE '' END))=lower($VALUE))`, filters.language);
  if (filters.english === 'true') where.push(`EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND p.english_taught=true)`);
  ...
  if (filters.csca === 'true') where.push(`EXISTS (SELECT 1 FROM programs p JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND ar.csca_required=true)`);
  if (filters.csca === 'false') where.push(`EXISTS (SELECT 1 FROM programs p LEFT JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND COALESCE(ar.csca_required,false)=false)`);
  ...
  if (filters.intake) add(`EXISTS (SELECT 1 FROM programs p JOIN intakes i ON i.program_id=p.id WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND i.intake_name ILIKE '%' || $VALUE || '%')`, filters.intake);
  if (filters.open === 'true') where.push(`EXISTS (SELECT 1 FROM programs p JOIN intakes i ON i.program_id=p.id WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND (i.application_status ILIKE '%open%' OR (i.application_open_date IS NOT NULL AND CURRENT_DATE >= i.application_open_date AND (i.application_deadline IS NULL OR CURRENT_DATE <= i.application_deadline))))`);
  ...
  return db.query(`SELECT u.id, u.name_english, u.name_chinese, u.short_name, u.university_type, u.city, u.province, u.country, u.official_website, u.admissions_website, u.c9, u.project_985, u.project_211, u.double_first_class, u.university_description, u.logo_url, u.cover_image_url,
    EXISTS (SELECT 1 FROM programs p WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND p.english_taught=true) AS has_english_program,
    ...
    EXISTS (SELECT 1 FROM programs p JOIN admission_requirements ar ON ar.program_id=p.id WHERE p.university_id=u.id AND COALESCE(p.is_active,true)=true AND ar.csca_required=true) AS has_csca,
