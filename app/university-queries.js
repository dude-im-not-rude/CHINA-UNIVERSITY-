...
export async function getUniversities(filters = {}) {
  ...
  const query = `
    SELECT u.id, u.name_english, u.name_chinese, u.short_name, u.university_type, u.city, u.province,
           u.country, u.official_website, u.c9, u.project_985, u.project_211, u.double_first_class,
           u.university_description, u.logo_url, u.cover_image_url,
...
export async function getUniversityById(id) {
  ...
  const programs = await db.query(`
    SELECT p.*, ar.csca_required, ar.csca_subjects, ar.english_requirement, ar.ielts_min, ar.toefl_min,
           ar.hsk_requirement, ar.minimum_percentage, ar.mathematics_required,
           COALESCE(json_agg(DISTINCT jsonb_build_object('name',i.intake_name,'open',i.application_open_date,'deadline',i.application_deadline,'status',i.application_status)) FILTER (WHERE i.id IS NOT NULL),'[]') AS intakes
    FROM programs p
    LEFT JOIN admission_requirements ar ON ar.program_id=p.id
  ...
  const scholarships = await db.query(`
    SELECT s.id, s.name, s.scholarship_type, s.provider, s.description, s.tuition_coverage, s.accommodation_coverage,
           s.stipend_coverage, s.application_deadline, s.official_website, us.notes
    FROM university_scholarships us JOIN scholarships s ON s.id=us.scholarship_id
    WHERE us.university_id=$1 AND COALESCE(us.available,true)=true ORDER BY s.name
  `, [id]);
...
export async function getScholarships() {
  const db = getDb();
  return db.query(`
    SELECT s.*, COUNT(DISTINCT us.university_id)::int AS university_count
    FROM scholarships s
    LEFT JOIN university_scholarships us ON us.scholarship_id=s.id AND COALESCE(us.available,true)=true
    GROUP BY s.id
    ORDER BY s.application_deadline NULLS LAST, s.name
  `);
}
