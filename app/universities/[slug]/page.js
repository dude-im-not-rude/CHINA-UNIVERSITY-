...
import { getShanghaiRanking } from '../../shanghai-ranking';
...
export default async function Page({ params }) {
  ...
  const ranking = await getShanghaiRanking(u.name_english);
  ...
  return <main><Header/>
    ...
    <section className="detail-grid"><div>
      ...
      <article className="panel"><h2>ShanghaiRanking</h2>{ranking.entries.length ? <div className="university-info-grid">{ranking.entries.map((entry)=><div className="university-info-card" key={`${entry.type}-${entry.year}`}><span className="label">{entry.type}</span><h3>#{entry.rank}</h3><p>{entry.year} ranking</p></div>)}</div> : <div className="university-empty">No numeric rank was found on the official ShanghaiRanking profile right now. We leave it blank instead of inventing a rank.</div>}<a className="btn secondary full" href={ranking.url} target="_blank" rel="noreferrer">Open official ShanghaiRanking profile ↗</a></article>
      ...
      <article className="panel"><h2>Campus & facilities</h2>{u.campuses.length ? <div className="university-info-grid">{u.campuses.map(c=><div className="university-info-card" key={c.id}><h3>{c.name || 'Campus'}</h3><span className="label">Location</span><p>{c.city || u.city || 'China'}{c.address ? ` · ${c.address}` : ''}</p>{c.description && <><span className="label">Facilities / notes</span><p>{c.description}</p></>}</div>)}</div> : <div className="university-empty">No structured campus record has been verified yet. The university city is shown above; detailed campus facilities will appear here once an official source record is added.</div>}</article>
    ...
    <aside>
      <div className="panel"><h3>Official links</h3>{u.official_website && <a className="btn primary full" href={u.official_website} target="_blank" rel="noreferrer">Official website ↗</a>}{u.admissions_website && <a className="btn secondary full" href={u.admissions_website} target="_blank" rel="noreferrer">Admissions ↗</a>}{ranking.url && <a className="btn secondary full" href={ranking.url} target="_blank" rel="noreferrer">ShanghaiRanking ↗</a>}</div>

      <div className="panel"><h3>University contact details</h3>{u.contacts.length ? <div className="university-info-grid">{u.contacts.map(c=><div className="university-info-card" key={c.id}><h3>{c.department || c.contact_type || 'Contact'}</h3>{c.email && <p><span className="label">Email</span><a href={`mailto:${c.email}`}>{c.email}</a></p>}{c.phone && <p><span className="label">Phone</span>{c.phone}</p>}{c.address && <p><span className="label">Address</span>{c.address}</p>}</div>)}</div> : <div className="university-empty">No structured contact record is available yet. Use the official university or admissions link below until a verified contact record is added.</div>}{u.official_website && <a className="btn secondary full" href={u.official_website} target="_blank" rel="noreferrer">Open official university site ↗</a>}{u.admissions_website && <a className="btn secondary full" href={u.admissions_website} target="_blank" rel="noreferrer">Open official admissions site ↗</a>}</div>
